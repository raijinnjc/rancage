...
interface PriorityDistrict {
  ...
  status: 'KRITIS' | 'TINGGI' | 'STABIL';
}

const PRIORITY_DATA: PriorityDistrict[] = [
  { rank: 1, id: '3206', name: 'Kabupaten Tasikmalaya', priorityScore: 94.2, typology: 'Kuadran IV (Miskin-Timpang)', p0: 12.11, within: 89.4, trend: -0.15, status: 'KRITIS' },
  { rank: 2, id: '3212', name: 'Kabupaten Indramayu', priorityScore: 92.8, typology: 'Kuadran IV (Miskin-Timpang)', p0: 12.77, within: 88.6, trend: -0.05, status: 'KRITIS' },
  { rank: 3, id: '3208', name: 'Kabupaten Kuningan', priorityScore: 91.5, typology: 'Kuadran IV (Miskin-Timpang)', p0: 12.82, within: 87.9, trend: 0.12, status: 'KRITIS' },
  { rank: 4, id: '3210', name: 'Kabupaten Majalengka', priorityScore: 88.6, typology: 'Kuadran IV (Miskin-Timpang)', p0: 11.94, within: 86.8, trend: -0.22, status: 'TINGGI' },
      ...
      <div className="border-b border-slate-50 dark:border-slate-900 px-5 py-4 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-[11px] text-slate-400 mt-0.5">
            Pengurutan komposit wilayah administratif yang membutuhkan penyesuaian fiskal struktural segera dan kalibrasi ulang penargetan.
          </p>
        </div>
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-slate-50/50 dark:bg-slate-900/35 border-b border-slate-100 dark:border-slate-900 text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">
              ...
              <th className="px-5 py-3 text-slate-400">Tipologi</th>
              <th className="px-5 py-3 cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-900/50 transition-colors text-right" onClick={() => handleSort('p0')}>
                <div className="flex items-center justify-end gap-1">
                  <span>Tingkat Kemiskinan P0</span>
                  {getSortIcon('p0')}
                </div>
              </th>
              <th className="px-5 py-3 cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-900/50 transition-colors text-right" onClick={() => handleSort('within')}>
                <div className="flex items-center justify-end gap-1">
                  <span>Disparitas Dalam Wilayah</span>
                  {getSortIcon('within')}
                </div>
              </th>
              ...
              >
                ...
                <td className="px-5 py-3 text-[10px]">
                  <span className={`px-2 py-0.5 rounded-sm font-semibold uppercase tracking-wider ${
                    row.typology.includes('Miskin-Timpang') 
                ...
                <td className="px-5 py-3 text-center">
                  <span className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider ${
                    ...
                    {row.status === 'KRITIS' && <AlertCircle className="h-2.5 w-2.5" />}
                    {row.status === 'TINGGI' && <AlertTriangle className="h-2.5 w-2.5" />}
                    {row.status === 'STABIL' && <CheckCircle className="h-2.5 w-2.5" />}
                    {row.status}
                  </span>
                </td>
              </tr>
